...
const TRIM_TARGET_BYTES = Math.floor(MAX_LOG_SIZE_BYTES * 0.6); // keep 60%
...
function trimLogFile(logPath: string, maxSize: number) {
  try {
    ...
    // Keep newest lines (from end) that fit within target
...
/**
 * DEV ONLY: Collects browser debug logs
 * POST /__manus__/logs -> writes to files
 */
...
export default defineConfig(() => {
  ...
  // ✅ IMPORTANT: Manus plugins only in DEV to avoid breaking GitHub Pages
  const plugins = [
    react(),
    tailwindcss(),
    jsxLocPlugin(),
  ...
  return {
    ...
    build: {
      // ✅ Pages artifact should publish this folder
